...
(function ($, window, document) {
    ...
    var Paginator = function (element, options) {
        ...
        // 最後のページ
        this.finalPage = Math.max(Math.ceil(this.totalItems / this.options.perPage), this.options.limitPagination);
    ...
    Paginator.prototype = {
        ...
        calcStartEndPage: function () {
            ...
            if (limit) {
                ...
                // 最大ページ数3ページのときは現在ページが5ページ以降のとき
                // 最終ページ+1-3から最終ページを表示する
                } else if (
                    this.currentPage + Math.floor(limit / 2) >=
                    this.finalPage
                ) {
            ...
            _self.ul.children().each(function () {
                ...
                switch (type) {
                    case 'number':
                        ...
                        break;
                    case 'prev':
                        page - 1 < _self.firstPage &&
                            _li.toggleClass('disabled');
                        break;
                    case 'next':
                        page + 1 > _self.finalPage &&
                            _li.toggleClass('disabled');
                        break;
                    default:
                        break;
